					...
					<div class="vipContBox">
						<div class="vipContTitleBox">
							<h1 class="vipContTitle">
								<span><img src="../resources/images/vipseversicon.png" /></span>尊敬的翡翠贷用户，以下是您在翡翠贷的已收款明细。
							</h1>
							<p>
								您目前的已收款总额是：<font>￥${count_money}</font>，共 <font>${count}</font> 笔收款。
							</p>
						</div>
						<form>
							<table cellpadding="0" cellspacing="0" border="0"
								...
								<c:forEach items="${loannlist}" var="loann">
									<tr>
										<td><a href="#">${loann.loanNumber}</a></td>
										<td>${loann.loanTitle}</td>
										<td>${loann.repayTime}</td>
										<td>${loann.borrowername}</td>
										<td>${loann.refundDeadline}</td>
										<td><fmt:formatNumber value="${loann.total-loann.tzyj}" pattern="0.00"/></td>
